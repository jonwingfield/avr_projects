...
#define AVR_PIN   PINC
#define AVR_PORT  PORTC
#define AVR_DDR   DDRC

// Temp defines for port to AVR
#define digitalRead(a)      (AVR_PIN & (a)) 
#define digitalWrite(a, c)      (c) ? (AVR_PORT |= 1 << (a)) : (AVR_PORT &= ~(1 << (a)))
#define INPUT 0
#define OUTPUT 1
#define pinMode(p, mode)   (mode) ? (AVR_DDR |= 1 << (p)) : (AVR_DDR &= ~(1 << (p)))

#define vw_tx_pin     PC5  
#define vw_ptt_pin	  PC6
...
extern uint8_t vx_tx_active(void);
...
// Sets a function to be called when recieve is complete (in interrupt context)
extern void vw_set_rx_message_callback(void (*func)(void));
